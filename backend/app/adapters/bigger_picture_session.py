"""Temporary anonymous Bigger Picture session for read-only operations."""
...
class BiggerPictureSessionError(Exception):
    """Session creation or a read-only operation failed."""


class BiggerPictureSession:
    ...
    async def __aenter__(self):
        if self._client is not None:
            raise BiggerPictureSessionError("Session is already open")
    ...
    async def _login(self):
        ...
        if not isinstance(token, str) or not re.fullmatch(r"[A-Za-z0-9._~+/=-]+", token):
            raise BiggerPictureSessionError("Bigger Picture session response is invalid")
        self._token = SecretStr(token)

    async def _read(self, path: str) -> list[dict]:
        if self._client is None:
            raise BiggerPictureSessionError("Use the session inside an async with block")
        try:
            for attempt in range(2):
                ...
                if (
                    ...
                    raise BiggerPictureSessionError("Bigger Picture read response is invalid")
                return data["list"]
        except (httpx.HTTPError, ValueError):
            self._token = None
            # Do not propagate response bodies or exceptions carrying Authorization headers.
            raise BiggerPictureSessionError("Bigger Picture session request failed") from None
